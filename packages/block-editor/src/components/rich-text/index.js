				...
				{ ( { isSelected, value, onChange, onFocus, Editable } ) => (
					<>
						...
						{ isSelected && hasFormats && (
							<FormatToolbarContainer inline={ inlineToolbar } anchorRef={ forwardedRef.current } />
						) }
...
const RichTextContainer = compose( [
	...
	withSelect( ( select, { clientId, instanceId, identifier = instanceId, isSelected } ) => {
		...
		const { __experimentalCanUserUseUnfilteredHTML, __experimentalUndo: undo } = getSettings();
